...
// ---------------------------------------------------------------------------
// Event adapters — thin wrappers producing structured Event from raw parsers
// ---------------------------------------------------------------------------

use crate::pm3::types::Event;

/// Parse `lf search` output into an Event.
/// Returns `CardDetected` with card data, or `NoCardFound`.
pub fn parse_lf_search_event(output: &str) -> Event {
    match parse_lf_search(output) {
        Some((card_type, card_data)) => Event::CardDetected {
            card_type,
            card_data,
        },
        None => Event::NoCardFound,
    }
}

/// Parse `hf search` output into an Event.
/// Returns `CardDetected` with card data, or `NoCardFound`.
pub fn parse_hf_search_event(output: &str) -> Event {
    match parse_hf_search(output) {
        Some((card_type, card_data)) => Event::CardDetected {
            card_type,
            card_data,
        },
...
/// Parse `lf t55xx detect` output into an Event.
/// Returns `T5577Status` with detection details.
pub fn parse_t5577_detect_event(output: &str) -> Event {
    let status = parse_t5577_detect(output);
    Event::T5577Status {
        detected: status.detected,
        password_set: status.password_set,
        chip_type: status.chip_type,
    }
}

/// Parse `hf mf autopwn` streaming line into an Event.
/// Returns `AutopwnProgress` for key discovery, `DumpComplete`/`DumpPartial`
/// for dump results, or `None` for non-matching lines.
pub fn parse_autopwn_line_event(line: &str) -> Option<Event> {
    let event = parse_autopwn_line(line)?;
    Some(match event {
        AutopwnEvent::DictionaryProgress { found, total } => Event::AutopwnProgress {
            phase: "dictionary".into(),
            keys_found: found,
            keys_total: total,
            elapsed_secs: 0,
        },
        AutopwnEvent::KeyFound { key: _ } => Event::AutopwnProgress {
            phase: "key_found".into(),
            keys_found: 0,
            keys_total: 0,
            elapsed_secs: 0,
        },
        AutopwnEvent::DumpComplete { file_path } => Event::DumpComplete { file_path },
        AutopwnEvent::DumpPartial { file_path } => Event::DumpPartial { file_path },
        AutopwnEvent::Failed { reason: _ } => Event::Error {
            message: "All key recovery attempts failed".into(),
        },
        AutopwnEvent::Finished { time_secs: _ } => return None,
        _ => return None,
    })
}

/// Parse clone verification output into an Event.
/// Returns `VerifyResult` with match status and any mismatched blocks.
pub fn parse_verify_event(source_uid: &str, clone_output: &str) -> Event {
    let (success, mismatched_blocks) = verify_match(source_uid, clone_output);
    Event::VerifyResult {
        success,
        mismatched_blocks,
    }
